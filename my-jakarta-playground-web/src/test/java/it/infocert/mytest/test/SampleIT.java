package it.infocert.mytest.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "my-jakarta-playground-ear.ear");

        // Current directory is the root of the "my-jakarta-playground-web" project. Go up one level
        // and enter the "my-jakarta-playground-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../my-jakarta-playground-ejb/target/my-jakarta-playground-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "my-jakarta-playground-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/my-jakarta-playground-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "my-jakarta-playground-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("it.infocert.mytest.test");
